var QC = function() {
    ...
    var $XML = function() {
        ...
        var loadXML = function(xmlString) {
            ...
            if (ieVer) if (XMLDoc.loadXML(xmlString)) return XMLDoc;
            else return null;
            else try {
    ...
    var $String = function() {
        ...
        return {
            escHTML: function(str) {
                var ret = str;
                for (var i = 0,
                         l = res.length; i < l; i++) ret = ret.replace(res[i], rep[i]);
                return ret
            },
    ...
    var $Event = function() {
        ...
        return {
            domReady: function(fun) {
                if (typeof fun == "function") if (ready.fired || document.readyState === "complete" || document.readyState === "loaded") {
                    ready();
                    fun()
    ...
    _pv.send = function(domain, path, opts) {
        ...
        if (t && t.length && t.length > 1) pgv_pvid = t[1];
            ...
            pgv_pvid = Math.round(Math.random() * 2147483647) * (new Date).getUTCMilliseconds() % 1E10;
    ...
    var reg = /\/qzone\/openapi\/qc(?:[^/_] * )\.js( ? :[ ? #] appId = (\d + )) ? /i;var scripts=document.getElementsByTagName("script");for(var i=0,script,l=scripts.length;i<l;i++){script=scripts[i];var src=script.src||"";var mat=src.match(reg);if(mat){qc_script=script;break}}var $Toolkit=_qc.Toolkit,$JSON=_qc.JSON,$XML=_qc.XML,$Object=_qc.Object,$QueryString=_qc.QueryString,$String=_qc.String,$Cookie=_qc.Cookie,$Console=_qc.Console;var EMPTY_FUN=function(){};var ieVer=window.ActiveXObject&&~~navigator.userAgent.match(/MSIE\s + (\d + ) / )[1];
    var config = {
    ...
    Proxy.dispatchReceive = function(seq, resText, status, fmt) {
        ...
        for (var i = 0; i < pendingRequests.length; i++) if (pendingRequests[i].seq == seq) {
    ...
    FLAProxy.getFlashHtml = function(flashArguments, requiredVersion, flashPlayerCID) {
        ...
        if (location && location.protocol.indexOf("https") < 0) _attrs.push(' codeBase="http://fpdownload.macromedia.com/get/flashplayer/current/swflash.cab#version=', requiredVersion, '"');
    ...
    var Login = function() {
        ...
        _insertButton._getPopupUrl = function(opts) {
            ...
            if (redirectURI) {
                if (redirectURI.indexOf("://") > 0) redirectURI = encodeURIComponent(redirectURI);
        ...
        var retFun = function(opts, loginFun, logoutFun, __outCallFlag) {
            ...
            if (!_user_info) access_token ? _getMe(function(openId) {
                if (openId) QC.api("get_user_info").success(function(req) {
                    _user_info = req.data;
                    loginFunFire()
                }).error(function(req) {
    ...
    _qc.init = function(cfg) {
        ...
        if (!TKObj.openid) {
            Proxy.invoke();
            if (fun_ready_pool.length && cfg.appId > -1) for (var i = 0; i < fun_ready_pool.length; i++) fun_ready_pool[i]();
            return
        }
        _qc.Login.getMe(function(oid, token, dt) {
                if (!~~dt.error && (dt.client_id <= 0 || dt.client_id % 1E6 == cfg.appId % 1E6)) {
    ...
    window.qc = QC;
    if (typeof Object.freeze == "function") Object.freeze(QC)
})();
/*  |xGv00|6683be6d8ed8e0185b5346fd6a2b2db5 */
